...
let permanentClientId: string | null = null;
...
async function raw<T = SoftOneResponse>(
  ...
  const url = requiredEnv("SOFTONE_URL");
...
async function authenticate(): Promise<string> {
  const login = await raw<SoftOneResponse>({
    service: "login",
    username: requiredEnv("SOFTONE_USERNAME"),
    password: requiredEnv("SOFTONE_PASSWORD"),
    appId: requiredEnv("SOFTONE_APPID"),
  });

  if (!login.success || !login.clientID) {
    throw new Error(
      `SoftOne login failed: ${String(login.error ?? "unknown error")}`,
    );
  }
  ...
  const auth = await raw<SoftOneResponse>({
    ...
    company: requiredEnv("SOFTONE_COMPANY"),
    branch: requiredEnv("SOFTONE_BRANCH"),
    module: process.env.SOFTONE_MODULE ?? "0",
    refid: requiredEnv("SOFTONE_REFID"),
  });

  if (!auth.success || !auth.clientID) {
    throw new Error(
      `SoftOne authenticate failed: ${String(
        auth.error ?? "unknown error",
      )}`,
    );
  }

  permanentClientId = auth.clientID;
...
async function withAuth<T = SoftOneResponse>(
  ...
  const clientID = permanentClientId ?? (await authenticate());

  let result = await raw<SoftOneResponse>({
    ...payload,
    clientID,
    appId: requiredEnv("SOFTONE_APPID"),
  });
  ...
  if (
    ...
    permanentClientId = null;

    const freshClientId = await authenticate();

    result = await raw<SoftOneResponse>({
      ...payload,
...
export const softoneCall = createTool({
  ...
  execute: async ({ service, payload }) => {
    if (["setData", "delData"].includes(service)) {
      return {
        blocked: true,
...
export const softoneRelations = createTool({
  ...
  description:
    "Returns outgoing and incoming SoftOne relationships from the cached relations file.",
  ...
  execute: async ({ object }) => {
    ...
    if (!existsSync(path)) {
      return {
        ...
        error:
          ...
          "Configure SOFTONE_RELATIONS_PATH or add the relations file.",
      };
    }
    ...
    return {
      ...
      outgoing: relations.filter(
        (relation) =>
          relation.from.toUpperCase() === objectName,
      ),

      incoming: relations.filter(
        (relation) =>
          relation.to.toUpperCase() === objectName,
      ),
    };
  },
});
