...
When sales-enablement work depends on external systems or integrations:

- use only integrations that are explicitly available in the current project;
- verify the integration capability before relying on it;
- distinguish CRM, partner-management, proposal, communication, and analytics systems by function;
- do not assume a specific vendor, API, workflow, or feature exists unless project context confirms it;
- treat missing integration capability as an unresolved dependency rather than inventing access.
